    ...
    if (attr != NULL) {
      ...
      if ((attr->cb_mem != NULL) && (attr->cb_size >= sizeof(StaticTask_t)) &&
        ...
        if ((attr->cb_mem == NULL) && (attr->cb_size == 0U) &&
            (attr->stack_mem == NULL)) {
          /* Control block and stack memory will be allocated from the dynamic
           * pool */
          mem = 0;
        }
      }
    ...
    if (mem == 1) {
...
#else
      hTask = xTaskCreateStaticAffinitySet(
          (TaskFunction_t)func, name, stack, argument, prio - 1U,
      ...
      if (mem == 0) {
        ...
        if (xTaskCreate((TaskFunction_t)func, name,
                        (configSTACK_DEPTH_TYPE)stack, argument, prio - 1U,
                        &hTask) != pdPASS) {
          hTask = NULL;
        }
#else
...
const char *osThreadGetName(osThreadId_t thread_id) {
  ...
  if (hTask == NULL) {
    ...
    /* Retrieve the name even though the function is not allowed to be called
     * from ISR */
  ...
  if ((IRQ_Context() == 0U) && (func != NULL)) {
    ...
    if ((attr != NULL) && (attr->cb_mem != NULL)) {
      if (attr->cb_size >= (sizeof(StaticTimer_t) + sizeof(TimerCallback_t))) {
        callb =
            (TimerCallback_t *)((uint32_t)attr->cb_mem + sizeof(StaticTimer_t));
      }
    }
#endif
    ...
    if (callb != NULL) {
      ...
      /*
        TimerCallback function is always provided as a callback and is used to
        call application specified function with its argument both stored in
        structure callb.
      */
      if (mem == 1) {
#if (configSUPPORT_STATIC_ALLOCATION == 1)
...
uint32_t osEventFlagsSet(osEventFlagsId_t ef_id, uint32_t flags) {
  ...
  if ((hEventGroup == NULL) || ((flags & EVENT_FLAGS_INVALID_BITS) != 0U)) {
    ...
    if (xEventGroupSetBitsFromISR(hEventGroup, (EventBits_t)flags, &yield) ==
      ...
      /* Retrieve bits that are already set and add flags to be set in current
       * call */
...
uint32_t osEventFlagsClear(osEventFlagsId_t ef_id, uint32_t flags) {
  ...
  if ((hEventGroup == NULL) || ((flags & EVENT_FLAGS_INVALID_BITS) != 0U)) {
    ...
    if (xEventGroupClearBitsFromISR(hEventGroup, (EventBits_t)flags) ==
      ...
      /* xEventGroupClearBitsFromISR only registers clear operation in the timer
       * command queue. */
      /* Yield is required here otherwise clear operation might not execute in
       * the right order. */
      /* See https://github.com/FreeRTOS/FreeRTOS-Kernel/issues/93 for more
       * info.               */
...
/*
  ...
  Limitations:
  - Priority inherit protocol is used by default, osMutexPrioInherit attribute
  is ignored.
...
osSemaphoreId_t osSemaphoreNew(uint32_t max_count, uint32_t initial_count,
  ...
  if ((IRQ_Context() == 0U) && (max_count > 0U) &&
  ...
  if ((IRQ_Context() == 0U) && (msg_count > 0U) && (msg_size > 0U)) {
    ...
    if (attr != NULL) {
      if ((attr->cb_mem != NULL) && (attr->cb_size >= sizeof(StaticQueue_t)) &&
        ...
        /* The memory for control block and message data is provided, use static
         * object */
        mem = 1;
      } else {
        if ((attr->cb_mem == NULL) && (attr->cb_size == 0U) &&
...
uint32_t osMessageQueueGetSpace(osMessageQueueId_t mq_id) {
  ...
  if (hQueue == NULL) {
    ...
    space =
        uxQueueGetQueueLength(hQueue) - uxQueueMessagesWaitingFromISR(hQueue);
...
__WEAK void vApplicationMallocFailedHook(void) {
  /* Assert when malloc failed hook is enabled but no application defined
   * function exists */
  configASSERT(0);
}
#endif
...
__WEAK void vApplicationStackOverflowHook(TaskHandle_t xTask,
  ...
  /* Assert when stack overflow is enabled but no application defined function
   * exists */
  configASSERT(0);
}
#endif
