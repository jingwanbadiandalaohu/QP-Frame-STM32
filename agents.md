...
## 来自 CLAUDE.md 的补充规则（筛选后）

以下为可直接用于当前项目的补充，默认遵循；若与上文冲突，以上文既有规则优先。

### 编码前上下文检索（最少闭环）

- 编码前至少完成3个相似实现阅读，并记录可复用点与约束。
- 先做依赖与集成点梳理：输入输出、配置来源、调用链路、任务/中断上下文。
- 涉及第三方库或框架时，优先查官方文档与现有工程用法，避免猜测式实现。
- 涉及通用功能实现时，先参考成熟开源实现，再落地到本项目风格。

### 实施与验证

- 采用“研究→计划→实施→验证”最小流程，先小步改动，再做本地验证。
- 每次改动至少执行一次最小验证（编译检查或关键路径自测）；验证失败则停止交付并说明原因。
- 验证说明必须可复现：写清命令、预期结果、实际结果。
- 若当前仓库缺少测试或工具，必须明确记录缺口与补偿计划。
...
- 技术维度：代码质量、测试覆盖、规范遵循。
- 战略维度：需求匹配、架构一致、风险评估。
- 综合评分与建议：0-100分，并给出“通过/退回/需讨论”。

### 风险与回退

- 涉及关键文件（如 `main.c`、启动流程、RTOS任务创建）时，必须给出影响面说明。
- 对破坏性改动必须提供迁移步骤或回退点。
- 连续三次验证失败时，暂停实现并回到需求与设计复盘。
